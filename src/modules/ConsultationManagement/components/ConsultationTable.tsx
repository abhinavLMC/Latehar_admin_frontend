...
import { CONSULTATION_BOOKING_ROUTE, JOIN_VIDEO_CALL_ROUTE, PRESCRIPTION_MANAGEMENT_ROUTE } from "@constants/AppConstant";
...
const ConsultationTable: React.FC<Props> = ({ data, loading, statusFilter, onRefresh }) => {
  ...
  const handleMarkComplete = async (consultationId: string) => {
    const hideLoading = message.loading('Marking consultation as complete...', 0);
    
    try {
      const response = await fetch(`${MOBILE_API_BASE_URL}/api/consultations/mark-complete/${consultationId}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to mark consultation as complete');
      }
      
      hideLoading();
      message.success('Consultation marked complete');
      onRefresh();
      
    } catch (error) {
      hideLoading();
      console.error('Error marking consultation complete:', error);
      message.error('Failed to mark consultation as complete. Please try again.');
    }
  };
  ...
  const baseColumns = [
    ...
    },
    {
      ...
      dataIndex: "isBooked",
      key: "isBooked",
      render: (isBooked: boolean) => (isBooked ? "Booked" : "Completed"),
    },
    {
      title: "Actions",
      key: "actions",
  ...
  const markCompleteColumn = {
    title: "Mark Complete",
    key: "mark_complete",
    render: (_: any, record: Consultation) => (
      <Button
        type="primary"
        onClick={() => handleMarkComplete(record.consultation_id)}
        aria-label="Mark consultation as complete"
        tabIndex={0}
        onKeyDown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            handleMarkComplete(record.consultation_id);
          }
        }}
      >
        Mark Complete
...
// Conditionally add both Join Call and Mark Complete columns
const columns = statusFilter === "completed" 
  ? baseColumns 
  : [...baseColumns, joinCallColumn, markCompleteColumn];
