...
import sidenavDataRaw from '@constants/menuData.json'
...
// Type assertion to ensure sidenavData has the correct type
const sidenavData = sidenavDataRaw as menuItems[];
...
const DynamicPageLayout = ({
  ...
  const getRouteSpecificMenu = (routeName: string): void => {
    ...
    if (menuObj?.pagemenu != null && Array.isArray(menuObj.pagemenu)) {
      setPageMenu([...menuObj.pagemenu, ...hasStaticPageMenuItem]);
    } else {
      setPageMenu([...hasStaticPageMenuItem]);
    }
    // set the title of found object
    if (menuObj != null && menuObj.title) {
      setTitle(menuObj.title);
    }
  };
    ...
    <>
      {isScrollable ? (
        <div
          ...
          className="settings-stepper title-row"
        >
          {!hideTitle ? (
          ...
          <Menu className="affix-menu">
            {linkedMenus?.map((obj) => (
              <Menu.Item
                key={obj.key}
                className={
                  obj.path?.split("/").at(-1) === currentPath
  ...
  const rightSideComponent = (
    <div className={styles["anchor-component"]}>{MainComp}</div>
  );
