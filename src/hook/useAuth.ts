  ...
  const submitHandler = async (values: { [key: string]: unknown }) => {
    ...
    try {
      ...
      if (res.data.status) {
        ...
        Cookies.set("username", result.username as string);

        const permissionsArray = (result.permission as PermissionType).Permissionmetadata.map((item: Permissionmetadata) => ({
...
export const logoutFunc = () => {
  ...
  Cookies.remove("role");
  // remove stored permissions at logout time
  localforage.removeItem(LOCAL_FORAGE_PERMISSION_KEY);
};
